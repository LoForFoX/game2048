...
public class Game2048Test {
    private final static Game game = new Game2048(new SquareBoard<>(4));
    public static void main(String[] args) {
        Board<Key, String> b2 = new SquareBoard<>(1);
        b2.fillBoard(asList("hello"));
        if (!"hello".equals(b2.getValue(b2.getKey(0 ,0)))) {
            throw new RuntimeException("board not work =(");
        }
        if (!b2.hasValue("hello")) {
            throw new RuntimeException("board not work =(");
        }
        ...
        game.init();
        if (b.availableSpace().size() != 14) {
            throw new RuntimeException("init must be add 2 item");
        }
        if (!game.canMove()) {
            throw new RuntimeException("canMove not work =(");
        }
        game.addItem();
        if (b.availableSpace().size() != 13) {
            throw new RuntimeException("addItem must be add 1 item");
        }
    }
